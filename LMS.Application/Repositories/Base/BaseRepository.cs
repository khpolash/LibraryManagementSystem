...
public class BaseRepository<T> : IBaseRepository<T> where T : AuditableEntity
{
    ...
    public IQueryable<T> GetAll() { return entities; }


    public async Task<List<T>> GetAllAsync() { return await entities.OrderByDescending(x => x.Id).ToListAsync(); }
    ...
    public T FirstOrDefault(long? id) { return entities.Find(id); }

    public async Task<T> FirstOrDefaultAsync(long? id) { return await entities.FindAsync(id); }
